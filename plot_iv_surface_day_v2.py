...
def load_day_data(csv_path: pathlib.Path, target: pd.Timestamp, chunksize: int) -> pd.DataFrame:
    ...
    day["log_moneyness"] = np.log(day["k_over_s"])
    day = day.replace([np.inf, -np.inf], np.nan)
    day = day.dropna(subset=["log_moneyness"])
    if len(day) < 10:
...
def build_regular_grid():
    gx = np.linspace(-0.22, 0.18, 25)
    gy = np.linspace(0.08, 1.0, 20)
...
def main():
    ...
    parser.add_argument("--n-days", type=int, default=0, help="Cate zile consecutive sa proceseze; 0 = toate zilele disponibile.")
    parser.add_argument("--outdir", type=pathlib.Path, default=pathlib.Path("plots_v2"))
    ...
    try:
        ...
        for i, day_ts in enumerate(selected_dates):
            ...
            x = day["log_moneyness"].to_numpy(dtype=float)
            y = day["T"].to_numpy(dtype=float)
            z = day["implied_vol"].to_numpy(dtype=float)
            grid_x, grid_y = build_regular_grid()
            ...
            print("Grid: x in [-0.22, 0.18] (25 pts), T in [0.08, 1.0] (20 pts)")
